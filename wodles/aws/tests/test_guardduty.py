...
def test_aws_guardduty_bucket_get_full_prefix(mock_wazuh_aws_integration, mock_sts, mock_service_prefix,
    ...
    with patch('guardduty.AWSGuardDutyBucket.check_guardduty_type', return_value=guardduty_native):
        ...
        if instance.type == "GuardDutyNative":
            assert os.path.join(instance.get_service_prefix(constants.TEST_ACCOUNT_ID), constants.TEST_REGION, '') == \
                   instance.get_full_prefix(constants.TEST_ACCOUNT_ID, constants.TEST_REGION)
        else:
            assert instance.prefix == instance.get_full_prefix(constants.TEST_ACCOUNT_ID, constants.TEST_REGION)
