...
def test_aws_service_initializes_properly(mock_wazuh_integration, mock_version, mock_path, mock_client, mock_connect,
    ...
    kwargs = utils.get_aws_service_parameters(db_table_name=constants.TEST_TABLE_NAME,
                                              service_name=constants.TEST_SERVICE_NAME, reparse=True,
                                              access_key=constants.TEST_ACCESS_KEY, secret_key=constants.TEST_SECRET_KEY,
                                              profile=constants.TEST_AWS_PROFILE, iam_role_arn=constants.TEST_IAM_ROLE_ARN,
                                              only_logs_after=constants.TEST_ONLY_LOGS_AFTER, region=constants.TEST_REGION,
                                              discard_field=constants.TEST_DISCARD_FIELD,
    ...
    mock_wazuh_integration.assert_called_with(instance, service_name=constants.TEST_SERVICE_NAME,
                                              ...
                                              skip_on_error=False)
    assert instance.service_name == constants.TEST_SERVICE_NAME
    assert instance.reparse
    assert instance.region == constants.TEST_REGION
    assert instance.only_logs_after == constants.TEST_ONLY_LOGS_AFTER
...
def test_aws_service_get_last_log_date(mock_wazuh_integration, mock_wazuh_aws_database, mock_sts):
    ...
    instance = utils.get_mocked_service(only_logs_after=constants.TEST_ONLY_LOGS_AFTER)
    assert instance.get_last_log_date() == constants.TEST_ONLY_LOGS_AFTER_WITH_FORMAT
